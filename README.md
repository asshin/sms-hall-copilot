...
面试一句话：短厅是有限动作空间。精确指令走规则，口语走意图 JSON，敏感办理必须确认，列表选择只能命中当前 BOSS 返回项，多项命中时保留原编号再选，余额和订购结果只来自工具。

## 文档

| 文档 | 内容 |
|------|------|
| [docs/DESIGN.md](docs/DESIGN.md) | 设计：规则优先、确认门、2way、语义对齐、多项筛选、配置人审 |
...
2. **多接口资费 2way**：`OFFER` → 查语言 → 查可订购列表 → 编号 / `第一个` / 名称 → `Y` → `msisdn+offer_id` 订购。关键词命中多项时筛出候选并保留原编号（如 `4`、`7`），再支持编号和语义选择。
