...
面试一句话：短厅是有限动作空间。精确指令走规则，口语走意图 JSON，敏感办理必须确认，列表选择只能命中当前 BOSS 返回项，多项命中时保留原编号再选，超时和重复 Y 不办理，余额和订购结果只来自工具。

## 文档

| 文档 | 内容 |
|------|------|
| [docs/DESIGN.md](docs/DESIGN.md) | 设计：规则优先、确认门、2way、多项筛选、会话超时、配置人审 |
...
2. **多接口资费 2way**：`OFFER` → 查语言 → 查可订购列表 → 编号 / `第一个` / 名称 → `Y` → `msisdn+offer_id` 订购。关键词命中多项时筛出候选并保留原编号（如 `4`、`7`）。选择/确认超时、重复 `Y`、余额不足、BOSS 超时都不编造成功。
...
| 号码 | 套餐 | 场景 |
|------|------|------|
| 85259990001 | 预付 / 中文 | 余额、暂停数据、资费订购、菜单充值、游戏积分 `GPNT` |
| 85259990002 | 后付 / 英文 | 账单；不能短厅暂停数据或金额充值 |
| 85259990003 | 预付 / 中文 | 数据已暂停、余额不足（订 88 档失败） |
| 85259990004 | 预付 / 中文 | 订购时 Mock BOSS 超时 |
...
| 多项筛选 | `OFFER` → `本地流量` | 只留下 `1`、`2`，编号不变；再回 `2` 订 10G |
| 筛后再选 | `OFFER` → `流量包` → `4` 或 `闲时流量包` | 留下 `4`、`7`，原编号仍可用 |
| 选不中 | `OFFER` → `天气` | 提示重选，不订购 |
| 重复确认 | `OFFER` → `1` → `Y` → `Y` | 第二次不再调订购工具 |
| 余额不足 | 换 003，`OFFER` → `1` → `Y` | 余额不足，未订购 |
| BOSS 超时 | 换 004，`OFFER` → `1` → `Y` | 系统繁忙，未订购 |
| 菜单充值 | `5` → `50` → `Y` | 预付金额入账 |
...
```
精确指令 / 菜单数字 / Y|N / 当前列表原编号  → 规则（不调 LLM）
awaiting_select / awaiting_confirm 超时        → session_expired，本轮 MO 不执行
办理成功后再发 Y                             → 回执，不再调工具
awaiting_select 且非唯一命中               → 序数/名称/纠错；多项则缩小列表（原 index 不变）
仍对不上且启用了 LLM                       → 只映射到当前可见列表项
未命中                                     → RAG + 意图 JSON（LLM 或启发式，白名单钳制）
敏感意图                                   → 只挂起，等 Y
工具失败 / 模型超时 / BOSS 失败              → 模板失败或降级，不编造办理结果
```
...
app/engine.py           会话与分发
app/flow.py             资费 2way
app/select_match.py     列表语义对齐与多项筛选
